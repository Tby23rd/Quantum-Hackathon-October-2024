...
# Define the HCOH Hamiltonian (simplified model)
def hcoh_hamiltonian():
    coeffs = np.array([
        0.7, 0.6, 0.6, 0.6,  # ZZ interactions for C=O double bond and C-H, O-H single bonds
        *[0.4] * num_qubits,  # Z on each qubit
        *[0.3] * (num_qubits - 1),  # XX interactions
        *[0.25] * (num_qubits - 1),  # YY interactions
        0.2, 0.2, 0.2,  # ZZZ interactions for H-C-O, H-C-H, and C-O-H angles
        0.15  # XXXX interaction for long-range correlations
    ])
    
    obs = [
        qml.PauliZ(0) @ qml.PauliZ(1),  # ZZ interaction for C=O double bond
        qml.PauliZ(0) @ qml.PauliZ(2),  # ZZ interaction for C-H single bond
        qml.PauliZ(0) @ qml.PauliZ(3),  # ZZ interaction for C-H single bond
        qml.PauliZ(1) @ qml.PauliZ(4),  # ZZ interaction for O-H single bond
    ]
    ...
    obs += [
        qml.PauliZ(0) @ qml.PauliZ(1) @ qml.PauliZ(2),  # ZZZ interaction for H-C-O angle
        qml.PauliZ(2) @ qml.PauliZ(0) @ qml.PauliZ(3),  # ZZZ interaction for H-C-H angle
        qml.PauliZ(0) @ qml.PauliZ(1) @ qml.PauliZ(4),  # ZZZ interaction for C-O-H angle
    ]
...
# Define a circuit for HCOH state preparation
@qml.qnode(dev)
def circuit(params):
