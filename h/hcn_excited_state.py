...
# Define the HCN Hamiltonian (simplified model)
def hcn_hamiltonian():
    coeffs = np.array([
        0.8, 0.7,  # ZZ interactions for C-N triple bond and C-H single bond
        *[0.4] * num_qubits,  # Z on each qubit
        *[0.3] * (num_qubits - 1),  # XX interactions
        *[0.25] * (num_qubits - 1),  # YY interactions
        0.2, 0.2,  # ZZZ interactions for H-C-N angle
        0.15  # XXXX interaction for long-range correlations
    ])
    
    obs = [
        qml.PauliZ(0) @ qml.PauliZ(1),  # ZZ interaction for C-N triple bond
        qml.PauliZ(1) @ qml.PauliZ(2),  # ZZ interaction for C-H single bond
    ]
    ...
    obs += [
        qml.PauliZ(0) @ qml.PauliZ(1) @ qml.PauliZ(2),  # ZZZ interaction for H-C-N angle
        qml.PauliZ(3) @ qml.PauliZ(4) @ qml.PauliZ(5),  # ZZZ interaction for additional correlation
    ]
...
# Define a circuit for HCN state preparation
@qml.qnode(dev)
def circuit(params):
