...
# Define the NEH+ Hamiltonian (simplified model)
def neh_plus_hamiltonian():
    coeffs = []
    obs = []

    # ZZ interactions for N-H bonds
    for i in range(1, 5):
        coeffs.append(0.6)
    ...
    # ZZZ interactions for H-N-H angles
    for i in range(1, 5):
        for j in range(i+1, 5):
            coeffs.append(0.2)
            obs.append(qml.PauliZ(0) @ qml.PauliZ(i) @ qml.PauliZ(j))
...
H = neh_plus_hamiltonian()

# Define a circuit for NEH+ state preparation
@qml.qnode(dev)
def circuit(params):
