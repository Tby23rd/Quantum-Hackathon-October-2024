...
# Define the C2H6 Hamiltonian (simplified model)
def c2h6_hamiltonian():
    coeffs = np.array([
        0.6,  # ZZ interaction for C-C single bond
        *[0.4] * num_qubits,  # Z on each qubit
        *[0.3] * (num_qubits - 1),  # XX interactions
        *[0.25] * (num_qubits - 1),  # YY interactions
        *[0.2] * 6,  # ZZZ interactions for C-H bonds
        0.15  # XXXX interaction for carbon atoms
    ])
    
    obs = [
        qml.PauliZ(0) @ qml.PauliZ(1),  # ZZ interaction for C-C single bond
    ]
    ...
    obs += [qml.PauliZ(0) @ qml.PauliZ(1) @ qml.PauliZ(i) for i in range(2, 8)]  # ZZZ interactions for C-H bonds
...
# Define a circuit for C2H6 state preparation
@qml.qnode(dev)
def circuit(params):
